...
pub const BinTable = struct {
    ...
    pub fn of(fits: *Fits, hdu: *Hdu) OpenError!BinTable {
        ...
        while (i < tfields) : (i += 1) {
            ...
            // Ownership of col_name/tdim has transferred into `columns`; disarm the loop
            // errdefers by clearing the locals so a later failure frees them via `columns`.
            col_name = null;
            tdim = null;
        }
        ...
        return .{ .fits = fits, .hdu = hdu, .columns = cols, .naxis1 = naxis1, .naxis2 = naxis2 };
    }
