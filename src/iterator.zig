...
//!
//! Generalization notes (design §19.2): ASCII-table iteration and per-column null-sentinel
//! substitution are documented follow-ups. The column reads/writes here use the underlying
//! `BinTable.readColumn`/`writeColumn` default options (raw nulls); a future revision can add
//! a per-`Binding` sentinel once the heterogeneous typing of sentinels is resolved. The image
//! walk already exposes null substitution through `ImageOpts.null_sentinel`.
            ...
            while (first_row < total_rows) {
                ...
                inline for (fields, 0..) |f, i| {
                    ...
                    if (roles[i] == .in or roles[i] == .inout) {
                        try table.readColumn(Elem, .{ .index = col_idx[i] }, first_row, v, .{});
                    }
                    @field(view, f.name) = v;
                }
                ...
                inline for (fields, 0..) |f, i| {
                    ...
                    if (roles[i] == .out or roles[i] == .inout) {
                        try table.writeColumn(Elem, .{ .index = col_idx[i] }, first_row, @field(view, f.name), .{});
                    }
                }
                first_row += n;
            }
...
const ColSpec = struct { tform: []const u8, ttype: []const u8 };
