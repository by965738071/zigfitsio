...
export abstract class BaseHDU {
  ...
  /** @internal Write the non-structural user header cards into a freshly created HDU. */
  _applyUserKeys(handle: bigint): void {
...
export class CompImageHDU extends ImageHDU {
  ...
  override _writeTo(handle: bigint, _primary: boolean): void {
    ...
    // Preserve EXTNAME (the general image path writes it via _applyUserKeys,
    // which this override does not call because the scanned header carries
