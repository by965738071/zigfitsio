/**
 * Neutral FFI IR shared by the bun:ffi and koffi backends.
 *
 * Every `zf_*` prototype is declared once against these neutral types (see
 * `lowlevel/protos.ts`); each backend maps them onto its own type system. Both
 * backends normalize values the same way so the rest of the package never
...
export interface NativeLibrary {
  readonly backend: "bun" | "koffi";
  /** Adapted `zf_*` callables keyed by symbol name. */
  readonly fn: Record<string, NativeFn>;
  /**
   * Copy `len` bytes at the raw address `ptr` and decode as UTF-8. Only used
   * for the single allocate-and-return API (`zf_read_key_longstr`); the copy
   * must happen before the matching `zf_free`.
   */
