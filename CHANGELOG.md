...
- **TypeScript/JavaScript bindings** (`bindings/typescript/`, npm package `zigfitsio`):
  ...
  `ZfOpenOpts`/`ZfScaling`/`ZfColInfo` codecs, LLP64-safe `long` handling) under an
  astropy-style high level (`open`/`HDUList`/HDU classes/`Header`/`Column`/`verify`, images
  as `FitsArray` TypedArrays, columnar `TableData` incl. VLA/complex/string columns, all
  compression codecs incl. lossy HCOMPRESS + quantized floats, WCS, checksums, gzip,
  in-memory files). Runs on **Bun** (`bun:ffi`) and **Node ≥18** (koffi) behind one FFI
  backend abstraction; includes a transparent workaround for a bun:ffi darwin-arm64
  stack-argument layout bug (bun ≤1.3.14, unreported upstream). 139 tests mirror the pytest
  suites (incl. the golden corpus) on both runtimes, plus a TS↔Python interop cross-check.
  Distribution: 7 cross-compiled `@zigfitsio/<platform>` packages (linux gnu/musl ×2 archs,
  macOS ×2, Windows x64) generated by `scripts/build-native.mjs`, published with the main
  package via npm trusted publishing in `.github/workflows/typescript.yml` (test matrix ×5
  OS in Debug + 3 ReleaseFast lanes).
